...
(defrule validate-profile-basic
    (declare (salience 90)) ;; Highest salience for validation
    ...
    (test (or (eq ?gender 0)       ;; Gender must be valid
              (eq ?gender 1) 
              (eq ?gender 2)))
=>
...
(defrule check-mbti
    (declare (salience 90)) ;; Ensure this fires early
...
(defrule calculate-self-image-score
    (declare (salience 70)) ;; Ensure this runs after MBTI
    ?profile <- (profile-input (user_id ?id) (self-image-score nil))
...
(defrule validate-notification-frequency
    ...
    (test (member$ ?freq (create$ 1 2 3))) ;; Validate frequency is 1, 2, or 3 hours
=>
...
(defrule print-profile
    ...
    ?profile <- (profile-input (user_id ?id)
                               ...
                               (self-image-score ?score&:(neq ?score nil))
                               (hobbies ?hobbies))
=>
